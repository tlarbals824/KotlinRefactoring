...
class TodoController(
    ...
    @GetMapping("/{id}")
    fun get(@PathVariable id: Long) = ResponseEntity.ok(TodoResponse.of(todoService.findById(id)))


    @PostMapping
    fun create(@RequestBody request: TodoRequest) = ResponseEntity.ok(TodoResponse.of(todoService.create(request)))
